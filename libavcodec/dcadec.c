...
static int set_channel_layout(AVCodecContext *avctx, int channels)
{
    ...
    if (s->amode < 16) {
        ...
        if (!s->xch_disable) {
            channels              = 2;
            s->output             = num_core_channels == 2 ? s->amode : DCA_STEREO;
                            ...
                            int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    downmix = avctx->ch_layout.nb_channels < full_channels;
    ...
    if (s->exss_ext_mask & DCA_EXT_EXSS_XLL) {
        ...
        if (s->xll_residual_channels > channels) {
            ...
            if (!s->xch_disable)
                avctx->ch_layout.nb_channels += s->xll_channels - s->xll_residual_channels;
        }
    }
