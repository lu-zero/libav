...
static av_cold int iss_read_header(AVFormatContext *s)
{
    ...
    av_channel_layout_default(&st->codecpar->ch_layout, stereo + 1);
    st->codecpar->sample_rate = 44100;
    if(rate_divisor > 0)
         st->codecpar->sample_rate /= rate_divisor;
    st->codecpar->bits_per_coded_sample = 4;
    st->codecpar->bit_rate = (stereo + 1) * st->codecpar->sample_rate
...
static int iss_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (av_channel_layout_check(&s->streams[0]->codecpar->ch_layout))
